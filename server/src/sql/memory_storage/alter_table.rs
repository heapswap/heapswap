use {
	super::memory_storage_lib::MemoryStorage,
...
impl AlterTable for MemoryStorage {
	async fn rename_schema(
		...
		/*
        let mut item = self.items.remove(table_name).ok_or_else(|| {
			AlterTableError::TableNotFound(table_name.to_owned())
		})?;
        */
        
        let (_, mut item) = self.items.remove(table_name).ok_or_else(|| {
			AlterTableError::TableNotFound(table_name.to_owned())
		})?;
	...
	async fn rename_column(
		...
		{
			return Err(AlterTableError::AlreadyExistingColumn(
	...
	async fn add_column(
		...
		let value = match (default, nullable) {
			(Some(expr), _) => {
				let evaluated =
					gluesql_core::executor::evaluate_stateless(None, expr)
						.await?;
			...
			(None, false) => {
				return Err(AlterTableError::DefaultValueRequired(
					column_def.clone(),
				)
				.into())
			}
...
//		for (_, row) in item.rows.iter_mut() {
            for (_, row) in item.rows.iter() {
				...
				DataRow::Map(_) => {
					return Err(Error::StorageMsg(
						"conflict - add_column failed: schemaless row found"
							.to_owned(),
					));
				}
			}
	...
	async fn drop_column(
		...
		match column_index {
			...
			None if if_exists => {}
			None => {
				return Err(AlterTableError::DroppingColumnNotFound(
