...
async fn main() -> Result<(), Box<dyn Error>> {
	...
	tokio::spawn(async move {
		...
		loop {
			...
			let message = crate::arr::random(32);
...
async fn test_swarm() -> Result<(), Box<dyn Error>> {
		...
		if i == 0 {
			...
			tokio::spawn(async move {
				...
				loop {
					...
					// Lock the mutex to access the swarm inside the async block
					let mut swarm = swarm_clone.lock().await;
					let message = arr::to_base32(&arr::random(16));
	...
	*/
	...
	/*
// this will block the thread until the swarm is locked
#[cfg(target_arch = "wasm32")]
pub fn get_mut_swarm(
	swarm: &SubfieldSwarm,
) -> Result<MutexGuard<Swarm<SubfieldBehaviour>>, SubfieldCreateSwarmError> {
	let swarm = match swarm.lock() {
		Ok(guard) => guard,
		Err(_) => {
			tracing::error!("Failed to lock swarm");
			return Err(SubfieldCreateSwarmError::FailedToLockSwarm);
		}
	};
	Ok(swarm)
}
...
// this will block the thread until the swarm is locked
#[cfg(not(target_arch = "wasm32"))]
pub async fn get_mut_swarm(
	swarm: &SubfieldSwarm,
) -> Result<MutexGuard<Swarm<SubfieldBehaviour>>, SubfieldCreateSwarmError> {
	let swarm = match swarm.lock().await {
		Ok(guard) => guard,
		Err(_) => {
			tracing::error!("Failed to lock swarm");
			return Err(SubfieldCreateSwarmError::FailedToLockSwarm);
