...
fn websys_websocket_to_gloo_websocket(
	ws: web_sys::WebSocket,
) -> Result<WebSocket, WebSocketError> {
	let ws: web_sys::WebSocket = ws
		.dyn_into::<web_sys::WebSocket>()
		.map_err(|_| WebSocketError::InvalidWebsocketObject)?;
	let ws: WebSocket = WebSocket::try_from(ws)
		.map_err(|_| WebSocketError::InvalidWebsocketObject)?;
	Ok(ws)
}
...
impl AsyncRead for WebSocketWrapper {
	fn poll_read(
		...
		match read.poll_next_unpin(cx) {
			...
			Poll::Ready(Some(Err(_))) => Poll::Ready(Err(std::io::Error::new(
