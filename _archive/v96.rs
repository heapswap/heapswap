...
impl V96 {
	pub fn new(version: u16, bytes: [u8; 12]) -> Self {
		Self {
			version,
			bytes,
			string: OnceCell::new(),
 ...
 * Randomable
*/
impl Randomable for V96 {
	fn random() -> Self {
		let bytes: [u8; 12] = arr::random(12).try_into().unwrap();
		Self::new(0, bytes)
	}
}
 ...
 * Stringable
*/
impl Stringable<VersionedBytesError> for V96 {
 ...
 * Vecable
*/
impl Vecable<VersionedBytesError> for V96 {
	fn from_arr(arr: &[u8]) -> Result<Self, VersionedBytesError> {
		...
		Ok(V96::new(version, bytes.try_into().unwrap()))
	}
 ...
 * Byteable
*/
impl Byteable<VersionedBytesError> for V96 {
 ...
 * Impls
*/
impl From<String> for V96 {
	fn from(string: String) -> Self {
		Self::from_string(&string).unwrap()
	}
}

impl From<&str> for V96 {
	fn from(string: &str) -> Self {
		V96::from_string(string).unwrap()
	}
}
...
impl Clone for V96 {
	fn clone(&self) -> Self {
		V96::new(self.version, self.bytes.clone())
	}
}
...
#[test]
fn test_v512() {
	let vec512 = V96::random();
	let vec512_serialized = serialize(&vec512).unwrap();
	// assert_eq!(vec512_serialized.len(), 12 + 2);
	assert!(vec512 == deserialize::<V96>(&vec512_serialized).unwrap());
}
