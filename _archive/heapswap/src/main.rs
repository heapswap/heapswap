...
pub struct AppState {
	...
	subfield_client: Arc<subfield::SubfieldClient>,
}
...
where
	F: FnOnce(Arc<subfield::SubfieldClient>) -> Fut + Send + 'static,
	Fut: std::future::Future<Output = EResult<()>> + Send,
{
	...
	let config = subfield::SubfieldConfig {
	...
	let subfield_client =
		Arc::new(subfield::SubfieldClient::new(config).await?);
