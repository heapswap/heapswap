...
enum EventOutcome {
	SubfieldSwarmEvent(SwarmEvent<swarm::SubfieldBehaviourEvent>),
	SubfieldRequest(SubfieldRequest),
	SubfieldResponse(SubfieldResponse),
}
...
pub async fn handle_events(
	...
	match event_outcome {
		...
		Some(EventOutcome::SubfieldRequest(request)) => {
			// handle_subfield_request(&mut store, &mut swarm, &mut tx, request).await?;
		}
		_ => {
