...
use super::events::{FromBehaviourToHandler, FromHandlerToBehaviour};

use super::{shared::Shared, upgrade::SubfieldUpgrade, OpenStreamError};
...
impl ConnectionHandler for Handler {
    // The message(s) a NetworkBehaviour can send to a ConnectionHandler via ToSwarm::NotifyHandler
    type FromBehaviour = FromBehaviourToHandler;
    // The message(s) a ConnectionHandler can send to a NetworkBehaviour via ConnectionHandlerEvent::NotifyBehaviour.
    type ToBehaviour = FromHandlerToBehaviour;
    // The inbound upgrade for the protocol(s) used by the handler.
    type InboundProtocol = SubfieldUpgrade;
    // The outbound upgrade for the protocol(s) used by the handler.
    type OutboundProtocol = SubfieldUpgrade;
    // Additional information returned from listen_protocol.
    type InboundOpenInfo = ();
    // Additional information passed to an OutboundSubstreamRequest.
    ...
    */
    ...
    /*
    Informs the handler about an event from the network.
    */
