...
impl SubfieldClient {
	...
	*/
	...
	// 	let Some(peer_id) =
	// 		self.closest_local_peer(&RoutingKey::random()).await?
	// 	else {
	...
	// 	s.write_all(&message_bytes)
	// 		.await
	// 		.map_err(|e| SubfieldError::FailedToWriteStream)?;

	// 	let mut buf = vec![0; message_len];
	// 	s.read_exact(&mut buf)
	// 		.await
	// 		.map_err(|e| SubfieldError::FailedToReadStream)?;

	// 	s.close()
	// 		.await
	// 		.map_err(|e| SubfieldError::FailedToCloseStream)?;
