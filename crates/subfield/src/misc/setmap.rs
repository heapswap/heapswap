...
struct SetMap<K: Eq + Hash + Clone + std::fmt::Debug, V: Eq + Hash + Clone + std::fmt::Debug> {
	kv_map: HashMap<K, HashSet<V>>,
	vk_map: HashMap<V, HashSet<K>>,
}

impl<K: Eq + Hash + Clone + std::fmt::Debug, V: Eq + Hash + Clone + std::fmt::Debug> SetMap<K, V> {
	pub fn new() -> Self {
		Self {
			kv_map: HashMap::new(),
			vk_map: HashMap::new(),
	...
	pub fn insert(&mut self, k: K, v: V) {
		self.kv_map.entry(k.clone()).or_insert_with(HashSet::new).insert(v.clone());
