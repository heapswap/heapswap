use crate::*;
use serde::{ser::Error, de::{self, Deserialize, Deserializer, Visitor}};
...
impl SubfieldRequest {
	pub fn new(proto: proto::subfield_request::RequestType) -> Self {
		Self { proto: proto::SubfieldRequest { request_type: Some(proto) } }
	}
...
impl Serialize for SubfieldRequest {
	fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
		where
			S: Serializer {
		serializer.serialize_bytes(proto::serialize(self.proto()).map_err(|e| S::Error::custom(e))?.as_ref())
	}
}
...
impl<'de> Visitor<'de> for SubfieldRequestVisitor {
    ...
    where
        E: de::Error,
    {
        let proto = proto::deserialize(Bytes::from(v.to_vec())).map_err(|e| E::custom(e))?;
...
impl SubfieldResponse {
	pub fn new(proto: proto::subfield_response::ResponseType) -> Self {
		Self { proto: proto::SubfieldResponse { response_type: Some(proto) } }
	}
...
impl<'de> Visitor<'de> for SubfieldResponseVisitor {
	...
	where
		E: de::Error,
	{
		let proto = proto::deserialize::<proto::SubfieldResponse>(Bytes::from(v.to_vec())).map_err(|e| E::custom(e))?;
