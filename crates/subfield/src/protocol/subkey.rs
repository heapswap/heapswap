...
impl Subkey {
	pub fn new(signer: Keyfield, cosigner: Keyfield, tangent: Keyfield) -> Self {
		Self { signer, cosigner, tangent }
	}
	...
	*/
	...
	fn hash_concat(hashes: &[&Option<V256>]) -> V256 {
		let concatenated: Vec<u8> = hashes.iter()
	...
	pub fn all_hashes(&self) -> Result<Vec<V256>, SubkeyError> {
		if !self.is_complete() {
	...
	pub fn is_complete(&self) -> bool {
		self.signer.is_some() && self.cosigner.is_some() && self.tangent.is_some()
	}
...
fn option_to_proto<T: Protoable<P, E>, P, E>(opt: Option<T>) -> Result<Option<P>, SubkeyError> {
	match opt {
		Some(t) => Ok(Some(t.to_proto().map_err(|_| SubkeyError::InvalidProto)?)),
		None => Ok(None),
	}		
}

fn key_to_proto(vb: &Option<V256>) -> Result<Option<proto::VersionedBytes>, SubkeyError> {
	option_to_proto::<V256, proto::VersionedBytes, VersionedBytesError>(vb.clone())
}

fn proto_to_option<T: Protoable<P, E>, P, E>(vb: Option<P>) -> Result<Option<T>, SubkeyError> {
	match vb {
		Some(p) => Ok(Some(T::from_proto(p).map_err(|_| SubkeyError::InvalidProto)?)),
		None => Ok(None),
	}		
}

fn proto_to_key(vb: &Option<proto::VersionedBytes>) -> Result<Option<V256>, SubkeyError> {
	proto_to_option::<V256, proto::VersionedBytes, VersionedBytesError>(vb.clone())
}
...
impl Protoable<proto::Subkey, SubkeyError> for Subkey {
	...
	fn from_proto_bytes(bytes: Bytes) -> Result<Self, SubkeyError> {
		Self::from_proto(proto::deserialize(bytes).map_err(|_| SubkeyError::DecodeError)?)
	}
