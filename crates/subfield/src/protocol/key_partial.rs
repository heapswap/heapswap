...
impl PartialKey {
	...
	*/
	...
	pub fn signer_is_signer(&self, signer: crypto::Keypair) -> Result<bool, KeyError> {
		if signer.public_key().versioned_bytes() == self.signer.clone().unwrap().versioned_bytes() {
			Ok(true)
		} else if signer.public_key().versioned_bytes() == self.cosigner.clone().unwrap().versioned_bytes() {
			Ok(false)
		} else {
