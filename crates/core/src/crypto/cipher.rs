...
impl Ciphering for Cipher{
    ...
    fn encrypt(&self, plaintext: Plaintext) -> Ciphertext {
        ...
        let encrypted_data = self.cipher.encrypt(&nonce.into(), plaintext.as_ref()).unwrap();
    ...
    fn decrypt(&self, ciphertext: Ciphertext) -> Plaintext {
        ...
        // Actual decryption
        let decrypted_data = self.cipher.decrypt(&nonce.into(), &ciphertext[NONCE_LENGTH..]).unwrap();
